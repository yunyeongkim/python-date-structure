...
class LinkedList:
    ...
    def print_list(self):
        ...
        while temp is not None:
            ...
            temp = temp.next # should be None at the last, but print None.
    ...
    def partition_list_solution(self,x):
        if not self.head:
            return None
    ...
    def partition_list(self,x):
        temp = self.head # it could be not first. 

        # Create separate LinkedList
        l1 = LinkedList(0)
        l2 = LinkedList(0)
 
        # Assign node to the separate List.
        while temp: # if temp.next is None terminate.
        ...
        # Return is no changes needed.
        if l1.length is None or l2.length is None:
            return None
        
        # Reset origin List
        self.make_empty()
        
        # Assign l1,l2 list to origin 
        
        l1_temp = l1.head.next
        l2_temp = l2.head.next
          
        while l1_temp :
            # Set Head first.
            if self.head is None:
                self.head = l1_temp
            self.append(l1_temp.value)
            # Move to the next nodes
            l1_temp = l1_temp.next
        
        while l2_temp :
            if l2_temp.next == None:
                self.tail = l2_temp
            print("value = ",l2_temp.value )
            self.append(l2_temp.value)
